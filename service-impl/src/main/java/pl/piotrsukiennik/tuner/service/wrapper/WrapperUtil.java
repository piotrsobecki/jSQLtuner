package pl.piotrsukiennik.tuner.service.wrapper;
...
public class WrapperUtil {
    ...
    public static void proceed( ShardService shardService, WriteQuery query, boolean rowsAffected ) {
        proceed( shardService, query, rowsAffected ? 1 : 0 );
    }

    public static void proceed( ShardService shardService, WriteQuery query, int rowsAffected ) {
    ...
    public static void log( Log log, QueryParsingNotSupportedException exception, String query ) {
        if ( log.isErrorEnabled() ) {
            log.error( String.format( PARSER_ERROR_FORMAT, query ), exception );
        }
    }

    public static ResultSet getResultSet( ShardService shardService, ReadQuery readQuery, DataSource rootDataSource ) throws SQLException {
        try {
