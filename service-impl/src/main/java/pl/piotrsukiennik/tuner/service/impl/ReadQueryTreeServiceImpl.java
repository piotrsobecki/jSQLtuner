...
public class ReadQueryTreeServiceImpl implements ReadQueryTreeService<ReadQuery> {
    ...
    @Override
    public Collection<ReadQuery> getReadQueries( Table table ) {
        return tree.get( getPath( table ) );
    }

    @Override
    public Collection<ReadQuery> getReadQueries( Column column ) {
        return tree.get( getPath( column ) );
    }

    @Override
    public Collection<ReadQuery> getReadQueriesByStar( Table table ) {
        return tree.get( getStarPath( table ) );
    }

    @Override
    public void submitReadQuery( ReadQuery query ) {
        if (query instanceof SelectQuery){
            putQuery( (SelectQuery)query, query );
        }
    }

    protected void putQuery( SelectQuery selectQuery, ReadQuery objectToStore ) {
